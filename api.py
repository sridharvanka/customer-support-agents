...
Endpoints:
  ...
  GET  /tickets   Ticket metadata (no PII) — used to populate the pre-run queue
...
@app.get("/tickets")
def list_tickets():
    """Return ticket metadata only — no PII, no account data."""
...
<style>
  :root {
    --bg: #0c0c0c; --bg2: #141414; --bg3: #1c1c1c; --border: #282828;
    --text: #e0e0e0; --text-mid: #999; --text-dim: #666;
    --green: #4ade80; --amber: #f59e0b; --red: #ef4444; --blue: #60a5fa;
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Queue */
  .queue-section { max-width: 860px; margin: 2rem auto 0; padding: 0 2rem; }
  .queue-header { font-size: 0.7rem; letter-spacing: 0.12em; text-transform: uppercase; color: var(--text-dim); margin-bottom: 0.75rem; }
  ...
  .queue-id { font-family: var(--font-mono); font-size: 0.65rem; color: var(--text-dim); min-width: 60px; }
  .queue-channel {
    font-family: var(--font-mono); font-size: 0.65rem; color: var(--blue);
    background: rgba(96,165,250,0.08); border: 1px solid rgba(96,165,250,0.2);
    padding: 0.1rem 0.35rem; flex-shrink: 0;
  }
  .queue-subject { font-size: 0.78rem; color: var(--text-mid); flex: 1; }
  ...
  .ticket-card {
    ...
    margin-bottom: 1rem; padding: 1.25rem 1.5rem; transition: border-color .15s;
  }
  ...
  .resolution-badge { font-size: 0.65rem; font-weight: 600; letter-spacing: 0.1em; text-transform: uppercase; padding: 0.25rem 0.6rem; flex-shrink: 0; }
  ...
  .intent-tag { font-size: 0.65rem; font-family: var(--font-mono); color: var(--blue); background: rgba(96,165,250,0.08); border: 1px solid rgba(96,165,250,0.2); padding: 0.15rem 0.45rem; }
  ...
  .agent-chip { font-size: 0.7rem; font-family: var(--font-mono); padding: 0.2rem 0.5rem; background: var(--bg3); border: 1px solid var(--border); color: var(--text-mid); }
  ...
  .ticket-notes { font-size: 0.75rem; color: var(--text-dim); font-family: var(--font-mono); line-height: 1.6; background: var(--bg); padding: 0.75rem; border-left: 2px solid var(--border); white-space: pre-wrap; word-break: break-word; }
  ...
  .triage-form textarea { background: var(--bg2); border: 1px solid var(--border); color: var(--text); font-family: var(--font-mono); font-size: 0.75rem; padding: 0.75rem; resize: vertical; min-height: 140px; outline: none; }
  .triage-form textarea:focus { border-color: #444; }
  .form-row { display: flex; gap: 0.75rem; align-items: center; }
  .triage-btn { background: transparent; border: 1px solid var(--border); color: var(--text-mid); font-family: var(--font-sans); font-size: 0.75rem; letter-spacing: 0.08em; text-transform: uppercase; padding: 0.55rem 1.2rem; cursor: pointer; transition: all .15s; }
  .triage-btn:hover { border-color: #555; color: var(--text); }
  .triage-result { margin-top: 0.75rem; }

  footer { border-top: 1px solid var(--border); padding: 1.5rem 2rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; margin-top: 3rem; }
...
<div class="hero">
  ...
  <p>An orchestrator-workers pipeline that classifies support tickets and routes them through specialist agents dynamically &mdash; with privacy-preserving pseudonymization built in from the start.</p>
...
<div class="queue-section" id="queueSection">
  <div class="queue-header" id="queueHeader">Loading tickets&hellip;</div>
...
function renderCard(t) {
  const intents = (t.intents || []).map(i => '<span class="intent-tag">' + escHtml(i) + '</span>').join("");
